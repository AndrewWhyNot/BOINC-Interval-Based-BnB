...
public:
	...
	*/
	...
	void setCurrentGlobMin(std::vector<T> globMinX, T globMinY) {
	        mGlobMinX.clear();
	        copy(globMinX.begin(), globMinX.end(), back_inserter(mGlobMinX));
        	mGlobMinY = globMinY;
		auto d = getDim();
        	while (mGlobMinX.size() != d) {
       			if (mGlobMinX.size() > d) {
			//for (int i = 0; i < mGlobMinX.size() - d; i++) {
        	        	mBounds.push_back(mBounds[0]);
			}
                        if (mGlobMinX.size() < d) {
		        //for (int i = 0; i < d - mGlobMinX.size(); i++) {
                		mBounds.pop_back();
            		}
			d = getDim();
        	}
